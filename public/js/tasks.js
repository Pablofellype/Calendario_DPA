...
function renderTaskList(dateKey) {
    ...
    dayTasks.forEach(task => {
        ...
        let uploadBtn = (state.user && canUpload) ? `<label class="w-24 h-24 sm:w-28 sm:h-28 rounded-[12px] border-2 border-dashed border-[#d1d5db] text-[#94a3b8] text-[10px] font-bold hover:text-[#e41e26] hover:border-[#e41e26]/40 hover:bg-[#fef2f2] flex flex-col items-center justify-center gap-1.5 cursor-pointer bg-[#fafafa] transition-all"><input type="file" class="hidden" accept="image/*" capture="environment" onchange="handleRealPhoto(this, '${task.id}')"><i data-lucide="camera" class="w-5 h-5"></i><span class="uppercase tracking-wider">Foto</span></label>` : '';
